...
with st.expander("📘 Metodologia e Fontes de Dados (Clique para abrir)"):
    ...
    **Período de Análise (2014-2023):**
    A série histórica apresentada encerra-se no ano de **2023**.
...
if paises_aviso_ativos:
    ...
    st.sidebar.warning(f"⚠️ **Atenção:** Para os países selecionados (**{lista_nomes}**), a participação do modal aquaviário é muito baixa (< 1%).")
...
if not df_plot.empty:
    ...
    # ==============================================================================
    # TABELA PERCENTUAL (DADOS DETALHADOS)
    # ==============================================================================
    st.divider()
    ...
    st.info("Selecione dois anos para comparar.")

    anos_disponiveis = [str(y) for y in range(2014, 2024)]
    col_var1, col_var2 = st.columns(2)
    ano_a = col_var1.selectbox("Ano Inicial:", anos_disponiveis, index=0, key="ano_a")
    ano_b = col_var2.selectbox("Ano Final:", anos_disponiveis, index=len(anos_disponiveis)-1, key="ano_b")

    if ano_a and ano_b:
        if int(ano_b) < int(ano_a):
            st.error("Ano Final deve ser maior que o Inicial.")
        else:
            ...
            df_diff = (df_b - df_a).dropna().sort_index(key=lambda col: col.map(normalizar_para_ordenacao))
            
            if not df_diff.empty:
                st.dataframe(df_diff.style.format("{:+.2f} p.p."), use_container_width=True, height=(len(df_diff)*35)+38)
            else:
                st.warning("Sem dados suficientes.")

    # ==============================================================================
    # TABELA DADOS ABSOLUTOS (NOVA LÓGICA)
    # ==============================================================================
    st.divider()
...
with st.expander("📊 Clique aqui para abrir a Tabela de Dados Absolutos"):
    if st.session_state.slider_principal == "Inicial":
        ...
        ano_str = str(st.session_state.slider_principal)
        if ano_str in df_tkm.columns:
            df_final = df_tkm[['Pais', 'Modal', ano_str]].copy()
            df_final = df_final.sort_values(by='Pais', key=lambda col: col.map(normalizar_para_ordenacao))
            st.markdown(f"Valores em **bilhões de tkm** para o ano **{ano_str}**.")
            st.dataframe(df_final.set_index('Pais'), use_container_width=True, height=(len(df_final)*35)+38)
        else:
            st.error(f"Dados para {ano_str} não encontrados.")

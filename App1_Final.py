...
# ==============================================================================
# 1. METODOLOGIA (TOPO) - TEXTO MELHORADO
# ==============================================================================
...
Belgica,Aquaviario -  Freight transport,0.159375,0.151659,0.147385,0.156507,0.119250,0.113178,0.109589,0.114958,0.111908,0.107042
Belgica,Ferroviario - Freight transport,0.110937,0.112164,0.110935,0.110379,0.122658,0.120930,0.115677,0.117729,0.123386,0.116901
Belgica,Rodoviario -  Freight transport,0.729688,0.736177,0.741680,0.733114,0.758092,0.765891,0.774734,0.767313,0.764706,0.776056
...
Mexico,Aquaviario -  Freight transport,0.079089,0.079185,0.078933,0.075898,0.077414,0.077468,0.071250,0.070759,0.070725,0.068863
Mexico,Ferroviario - Freight transport,0.246555,0.245415,0.242697,0.241839,0.235169,0.231355,0.235458,0.239078,0.236423,0.224919
Mexico,Rodoviario -  Freight transport,0.674356,0.675400,0.678371,0.682263,0.687417,0.691176,0.693293,0.690163,0.692852,0.706217
...
Russia,Aquaviario -  Freight transport,0.042993,0.042998,0.039994,0.042004,0.034999,0.036006,0.035985,0.037990,0.037012,0.039991
Russia,Ferroviario - Freight transport,0.863998,0.869008,0.868020,0.869004,0.876997,0.871999,0.871024,0.865019,0.860994,0.844006
Russia,Rodoviario -  Freight transport,0.093008,0.087994,0.091986,0.088992,0.088004,0.091994,0.092991,0.096991,0.101994,0.116003
...
Áustria,Rodoviario -  Freight transport,0.633846,0.647951,0.649485,0.651429,0.662500,0.665295,0.678414,0.680000,0.680683,0.689000
"""
...
# ==============================================================================
# 2. DICA DE NAVEGAÇÃO (MOVIDO PARA CÁ) - COM A CORREÇÃO SOLICITADA
# ==============================================================================
st.divider()
st.info("""
💡 **Dicas de Navegação:**
...
# ==============================================================================
# 3. BOTÃO E LINHA DO TEMPO (MOVIDO PARA CÁ)
# ==============================================================================
...
if not df_plot.empty:
    ...
    # [LÓGICA DE TAMANHO VISUAL]
    df_plot['Tamanho_Visual'] = df_plot['Aquaviário (%)'].apply(lambda x: 1 if x < 1 else x)
    
    # [NOVO] COLUNA DE TEXTO FORMATADO ("15%" se inteiro, "15.25%" se decimal)
    ...
    # st.divider()  <-- Removido daqui pois já temos divisões acima
    ...
    fig = px.scatter(
        ...
        # HOVER_DATA USA A COLUNA DE TEXTO FORMATADO
        hover_data=['Aquaviário_Texto', 'Ano']
    )
    
    # --- AJUSTE DA LEGENDA DO PLOTLY PARA BAIXO ---
    fig.update_layout(
        ...
        # CONFIGURAÇÃO DA LEGENDA HORIZONTAL NA BASE
        legend=dict(
            orientation="h",       # Horizontal
            yanchor="top",
            y=-0.25,               # Abaixo do gráfico (ajuste se necessário)
            xanchor="center",
            x=0.5,
            title_text=None        # Remove título da legenda para limpar o visual
        ),
        
        height=650,
        margin=dict(b=150),        # Aumenta margem inferior para caber a legenda
        showlegend=True,
    ...
    fig.update_traces(
        ...
        # USAMOS A COLUNA DE TEXTO FORMATADO (ÍNDICE 0)
        hovertemplate="<b>%{text}</b> (%{customdata[1]})<br><br>" +
                      "🚂 Ferroviário: %{x}%<br>" +
                      "🚛 Rodoviário: %{y}%<br>" +
                      "🚢 Aquaviário: %{customdata[0]}<extra></extra>"
    )
    
    st.plotly_chart(fig, use_container_width=True)
    
    # LEGENDA DESCRITIVA (OPCIONAL - COLOCADA APÓS O GRÁFICO SEGUINDO A ORDEM)
    st.caption("""
    **Legenda dos Eixos:**
    ...
    * **Tamanho da Bolinha:** Participação do Modal **Aquaviário** (%)
    ...
    # APLICAR FORMATAÇÃO INTELIGENTE NA TABELA TAMBÉM
    st.dataframe(
        df_plot[cols_to_show].sort_values(by='Pais', key=lambda col: col.map(normalizar_para_ordenacao))
        .set_index('Pais')
        .style.format(format_pct_text), # Formatação inteligente aplicada aqui
        use_container_width=True,
        height=altura_tabela
    )

    # --- 6. SEÇÃO: ANÁLISE DE VARIAÇÃO ---
    st.divider()
    st.subheader("📈 Análise de Variação (Comparativo entre Anos)")

    st.info("Selecione dois anos abaixo para visualizar como a participação de cada modal mudou (em pontos percentuais) para os países exibidos (Principais + Selecionados).")

    anos_disponiveis = [str(y) for y in range(2014, 2024)]
    col_var1, col_var2 = st.columns(2)

    with col_var1:
        ano_a = st.selectbox("Ano Inicial (Base):", anos_disponiveis, index=0, key="ano_a") # 2014
    with col_var2:
        ano_b = st.selectbox("Ano Final (Comparação):", anos_disponiveis, index=len(anos_disponiveis)-1, key="ano_b") # 2023

    if ano_a and ano_b:
        # VALIDAÇÃO DO ANO FINAL < ANO INICIAL
        if int(ano_b) < int(ano_a):
            st.error("⚠️ **Erro:** O Ano Final não pode ser anterior ao Ano Inicial. Por favor, ajuste a seleção.")
        else:
            # Filtrar dados para os países selecionados no sidebar + principais
            df_var = df[df['Pais'].isin(paises_para_mostrar)]
            
            # Obter dados dos dois anos
            df_a = df_var[df_var['Ano'].astype(str) == ano_a].set_index('Pais')[['Rodoviário (%)', 'Ferroviário (%)', 'Aquaviário (%)']]
            df_b = df_var[df_var['Ano'].astype(str) == ano_b].set_index('Pais')[['Rodoviário (%)', 'Ferroviário (%)', 'Aquaviário (%)']]
            
            # Calcular a diferença (Ano B - Ano A)
            df_diff = df_b - df_a
            df_diff = df_diff.dropna()
            
            # Ordenar alfabeticamente
            df_diff = df_diff.sort_index(key=lambda col: col.map(normalizar_para_ordenacao))
            
            # Exibir Tabela com Formatação (+/-) e altura dinâmica
            if not df_diff.empty:
                altura_diff = (len(df_diff) * 35) + 38
                st.dataframe(
                    df_diff.style.format("{:+.2f} p.p."), # Mantido 2 casas para variação
                    use_container_width=True,
                    height=altura_diff
                )
            else:

...
# ==============================================================================
# 1. DADOS DO CENÁRIO INICIAL (EDITÁVEIS)
# ==============================================================================
# ATENÇÃO: Ajuste estes valores para igualar a posição das bolhas da sua imagem.
# Ordem: [Ferroviário (X), Rodoviário (Y), Aquaviário (Tamanho)]
# ==============================================================================
DADOS_CENARIO_INICIAL = [
    # Pais       | Ferr(X) | Rod(Y) | Aqua(Size)
    ('Russia',     85.0,     10.0,    5.0),   # Rússia (Muita Ferrovia)
    ('Canada',     60.0,     40.0,    0.0),   # Canadá
    ('USA',        40.0,     45.0,    15.0),  # EUA (Bem distribuído)
    ('China',      30.0,     50.0,    20.0),  # China
    ('Brasil',     21.0,     62.0,    17.0),  # Brasil (Muito Rodoviário)
    ('Alemanha',   23.0,     65.0,    12.0),  # Alemanha
    ('Hungria',    20.0,     75.0,    5.0),   # Hungria
    ('França',     15.0,     80.0,    5.0),   # França
    ('Belgica',    10.0,     75.0,    15.0),  # Bélgica
    ('Dinamarca',  10.0,     80.0,    10.0)   # Dinamarca
]
...
# ==============================================================================
# 2. DADOS REAIS (2014-2023) - DA SUA PLANILHA (NÃO MEXER)
# ==============================================================================
...
def carregar_dados_completos():
    ...
    modal_map = {
        ...
        'Rodoviario - Freight transport': 'Rodoviário (%)',
        'Aquaviario -  Freight transport': 'Aquaviário (%)', # Acento corrigido
...
# Função que o botão chama para forçar a atualização
...
with col_btn:
    st.write("### Ação")
    # Botão atualiza o estado e o Streamlit detecta a mudança
...
with col_slider:
    ...
    selecao = st.select_slider(
        ...
        key="slider_tempo" # Chave diferente para evitar conflito direto, controlamos via value
    )
    
    # Sincronização manual (se o usuário arrastar o slider)
...
if st.session_state.timeline_pos == "Inicial":
    titulo = "Cenário Inicial (Referência Visual)"
    df_plot = df[df['Ano'] == 'Inicial']
else:
    ano = st.session_state.timeline_pos
...
if not df_plot.empty:
    ...
    with st.expander(f"📋 Ver Dados Tabela ({st.session_state.timeline_pos})"):
        st.dataframe(df_plot.set_index('Pais')[['Ferroviário (%)', 'Rodoviário (%)', 'Aquaviário (%)']])
else:
    st.error("Erro: Dados não encontrados.")
...
with st.expander("📘 Metodologia e Fontes de Dados (Clique para abrir)"):
    st.markdown("""
    **Origem e Veracidade dos Dados:** Dados reais extraídos de relatórios governamentais (Eurostat, EPL, BTS, NBSC, Rosstat).
    **Métrica:** Toneladas-quilômetro (tkm).
    **Fórmula:** $\\text{Participação (\\%)} = (\\text{tkm do Modal} / \\text{Total tkm}) \\times 100$
    """)
...
cores_paises = {
    'Alemanha': '#FFCE00', 'Bélgica': '#4B0082', 'Brasil': '#009739', 'Canadá': '#FF0000', 'China': '#DAA520', 
    'Dinamarca': '#C8102E', 'EUA': '#002868', 'França': '#002395', 'Hungria': '#436F4D', 'Rússia': '#FF8C00', 
    'Argentina': '#d8ccdd', 'Austrália': '#2e8ec1', 'Áustria': '#944d33', 'Azerbaijão': '#5cefcb', 'Bulgária': '#e408c2', 
    'Colômbia': '#561827', 'Coreia do Sul': '#a0e6c9', 'Croácia': '#183c9e', 'Eslováquia': '#94152a', 'Espanha': '#ab7538', 
    'Finlândia': '#0f8d9f', 'Holanda': '#fa3d56', 'Itália': '#831a10', 'Japão': '#9e96e7', 'Luxemburgo': '#d0970b', 
    'Reino Unido': '#0792fa', 'Romênia': '#8e83b0', 'Sérvia': '#a3d636', 'Suécia': '#b7bb68', 'Vietnã': '#4a110b',
    'República Tcheca': '#11457e', 'Polônia': '#dc143c', 'México': '#006847', 'Turquia': '#e30a17', 'Chile': '#0039a6', 'Noruega': '#ba0c2f'
}

PAISES_PRINCIPAIS = sorted(['Alemanha', 'Bélgica', 'Brasil', 'Canadá', 'China', 'Dinamarca', 'EUA', 'França', 'Hungria', 'Rússia'], key=normalizar_para_ordenacao)

# --- BASE DE DADOS 1: PERCENTUAIS (GLOBAL) ---
...
# --- BASE DE DADOS 2: TKM ABSOLUTO (GLOBAL) ---
...
def carregar_dados_completos():
    ...
    # 2. Criar e concatenar dados iniciais
...
# --- CONTROLE DE ESTADO ---
if "slider_principal" not in st.session_state: st.session_state.slider_principal = "Inicial"
def atualizar_para_recente(): st.session_state.slider_principal = "2023"

# --- SIDEBAR ---
st.sidebar.header("Configurações")
is_inicial = (st.session_state.slider_principal == "Inicial")
todos_os_paises = df['Pais'].unique().tolist()
paises_adicionais_disponiveis = sorted([p for p in todos_os_paises if p not in PAISES_PRINCIPAIS], key=normalizar_para_ordenacao)

if is_inicial: st.sidebar.warning("🔒 **Nota:** A adição de países extras só está disponível para a série histórica (2014-2023).")
selecao_adicional = st.sidebar.multiselect("Adicionar Outros Países:", options=paises_adicionais_disponiveis, default=[], disabled=is_inicial)
...
if paises_aviso_ativos: st.sidebar.warning(f"⚠️ **Atenção:** Países com baixa participação aquaviária (< 1%): {', '.join(paises_aviso_ativos)}")

paises_para_mostrar = sorted(PAISES_PRINCIPAIS + (selecao_adicional if not is_inicial else []), key=normalizar_para_ordenacao)

# --- CORPO PRINCIPAL ---
st.divider()
st.info("💡 **Dica:** Clique no nome do país na legenda para ocultá-lo/exibi-lo.")
...
df_plot = df[(df['Ano'].astype(str) == st.session_state.slider_principal) & (df['Pais'].isin(paises_para_mostrar))]
...
if not df_plot.empty:
    ...
    fig = px.scatter(
        df_plot, x="Ferroviário (%)", y="Rodoviário (%)", size="Tamanho_Visual", color="Pais", text="Pais", 
        color_discrete_map=cores_paises, title=f"<b>Cenário: {st.session_state.slider_principal}</b>", 
        size_max=60, template="plotly_white", range_x=[-5, 105], range_y=[-5, 105], category_orders={"Pais": paises_para_mostrar},
        hover_data=['Aquaviário_Texto', 'Ano']
    )
    fig.update_layout(xaxis=dict(title="<b>Eixo X:</b> Ferroviário (%)", dtick=10), yaxis=dict(title="<b>Eixo Y:</b> Rodoviário (%)", dtick=10), legend=dict(orientation="h", yanchor="top", y=-0.25, xanchor="center", x=0.5), height=650, margin=dict(b=150))
    fig.update_traces(textposition='top center', marker=dict(line=dict(width=1, color='DarkSlateGrey'), opacity=0.9), hovertemplate="<b>%{text}</b> (%{customdata[1]})<br><br>🚂 Ferroviário: %{x}%<br>🚛 Rodoviário: %{y}%<br>🚢 Aquaviário: %{customdata[0]}<extra></extra>")
    st.plotly_chart(fig, use_container_width=True)

    st.divider()
    st.subheader(f"📋 Dados Detalhados (Percentuais): {st.session_state.slider_principal}")
    st.dataframe(df_plot[['Pais', 'Rodoviário (%)', 'Ferroviário (%)', 'Aquaviário (%)']].set_index('Pais').style.format(format_pct_text), use_container_width=True)

    st.divider()
    st.subheader("📈 Análise de Variação")
    c1, c2 = st.columns(2)
    ano_a = c1.selectbox("Ano Inicial:", [str(y) for y in range(2014, 2024)], index=0)
    ano_b = c2.selectbox("Ano Final:", [str(y) for y in range(2014, 2024)], index=9)
    
    if ano_a and ano_b:
        if int(ano_b) < int(ano_a): st.error("Ano Final deve ser maior que o Inicial.")
        else:
            d = df[df['Pais'].isin(paises_para_mostrar)]
            da = d[d['Ano'].astype(str)==ano_a].set_index('Pais')[['Rodoviário (%)', 'Ferroviário (%)', 'Aquaviário (%)']]
            db = d[d['Ano'].astype(str)==ano_b].set_index('Pais')[['Rodoviário (%)', 'Ferroviário (%)', 'Aquaviário (%)']]
            st.dataframe((db-da).dropna().style.format("{:+.2f} p.p."), use_container_width=True)

    # --- TABELA DE DADOS ABSOLUTOS ---
    st.divider()
    ...
    with st.expander("📊 Clique aqui para abrir a Tabela de Dados Absolutos"):
        if st.session_state.slider_principal == "Inicial":
            st.warning("⚠️ Os dados absolutos não estão disponíveis para o Cenário Inicial. Selecione um ano específico.")
        else:
            ...
            ano_sel = str(st.session_state.slider_principal)
            if ano_sel in df_tkm.columns:
                df_final = df_tkm[['Pais', 'Modal', ano_sel]].copy().sort_values(by='Pais', key=lambda col: col.map(normalizar_para_ordenacao))
                st.markdown(f"Valores em **bilhões de tkm** para o ano **{ano_sel}**.")
                st.dataframe(df_final.set_index('Pais'), use_container_width=True, height=(len(df_final)*35)+38)
            else:
                st.error(f"Dados para {ano_sel} não encontrados.")
else:

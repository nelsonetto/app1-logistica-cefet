...
<style>
    ...
    /* Estilo para a mensagem de bloqueio */
    .stAlert {
        font-size: 14px;
        line-height: 1.4;
    }
...
# MÉTRICA DE ANO EM DESTAQUE E AVISO VISUAL
col_aviso, col_ano = st.columns([3, 1])
with col_aviso:
    st.info("💡 **Dica:** Clique no nome do país na legenda (à direita) para **ocultá-lo** ou dê um **duplo clique** para isolá-lo.")
with col_ano:
    st.metric(label="Ano Visualizado", value=st.session_state.slider_principal)
...
if not df_plot.empty:
    ...
    fig.update_traces(
        ...
        hovertemplate="<b>%{text}</b><br><br>" +
                      "🚂 Ferroviário: %{x}%<br>" +
                      "🚛 Rodoviário: %{y}%<br>" +
                      "🚢 Aquaviário (Tamanho): %{marker.size}%<extra></extra>"
    )

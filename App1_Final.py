...
<style>
    ...
    /* CORREÇÃO DAS CAIXAS DE DICA (st.info / st.warning / st.error) */
    div[data-baseweb="notification"] {
        background-color: #00529B !important; 
        border-color: #00529B !important;
    }
    /* Exceção para mensagem de ERRO (Fundo Vermelho) */
...
if not df_plot.empty:
    ...
    * **Tamanho da Bolinha:** Participação do Modal **Aquaviário** (%)
    ...
    # LEGENDA DINÂMICA DO ANO SELECIONADO
    ...
    fig = px.scatter(
        ...
        # HOVER_DATA COM 'ANO' ADICIONADO
        hover_data={
            "Pais": False,
            "Tamanho_Visual": False,
            "Ano": True, # Exibe o Ano no tooltip
            "Ferroviário (%)": ":.1f",
            "Rodoviário (%)": ":.1f",
            "Aquaviário (%)": ":.1f"
        }
    ...
    fig.update_traces(
        ...
        hovertemplate="<b>%{text}</b> (%{customdata[2]})<br><br>" + # Mostra País (Ano)
                      "🚂 Ferroviário: %{x}%<br>" +
                      "🚛 Rodoviário: %{y}%<br>" +
                      "🚢 Aquaviário: %{customdata[5]}%<extra></extra>"
    )

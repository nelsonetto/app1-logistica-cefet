...
def carregar_dados_completos():
    ...
    for col in ['Ferroviário (%)', 'Rodoviário (%)', 'Aquaviário (%)']:
        if col in df_real.columns:
            ...
            # Aplica a correção visual para não zerar (0.1%)
...
with col_slider:
    ...
    # SLIDER PRINCIPAL: Mantido como st.select_slider (bolas/pontos com rótulos)
    ...
    * **Tamanho da Bolinha:** Participação do Modal **Aquaviário** (%)
    ...
    fig.update_traces(
        ...
        hovertemplate="<b>%{text}</b> (%{customdata[1]})<br><br>" +
                      "🚂 Ferroviário: %{x}%<br>" +
                      "🚛 Rodoviário: %{y}%<br>" +
                      "🚢 Aquaviário: %{customdata[0]:.2f}%<extra></extra>"
    )
    ...
    # --- 6. NOVA SEÇÃO: ANÁLISE DE VARIAÇÃO (REVERTIDA PARA SELECTBOXES) ---
    st.divider()
    ...
    if ano_a and ano_b:
        ...
        if int(ano_b) < int(ano_a):
            ...
            if not df_diff.empty:
                ...
                st.dataframe(
                    df_diff.style.format("{:+.2f} p.p."), # 2 casas decimais
                    use_container_width=True,
                    height=altura_diff
                )
            else:

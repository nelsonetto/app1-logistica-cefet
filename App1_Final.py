...
# ==============================================================================
# 2. METODOLOGIA COMPLETA (RESTAURADA)
# ==============================================================================
    ...
    with st.expander("📊 Clique aqui para abrir a Tabela de Dados Absolutos"):
        if st.session_state.slider_principal == "Inicial":
            ...
            if ano_selecionado in df_tkm.columns:
                # Pivot para formato "Wide" (1 linha por país, colunas = modais)
                df_slice = df_tkm[['Pais', 'Modal', ano_selecionado]].copy()
                df_pivot = df_slice.pivot(index='Pais', columns='Modal', values=ano_selecionado)
                
                # Resetar index para ter 'Pais' como coluna, se desejar (ou manter como index)

...
if not df_plot.empty:
    ...
    * **Tamanho da Bolinha:** Participação do Modal **Aquaviário** (%)
    ...
    # ==============================================================================
    # 7. TABELA DE DADOS ABSOLUTOS (NOVA SEÇÃO ADICIONADA)
    # ==============================================================================
    st.divider()
    st.subheader("📋 Dados Absolutos: Toneladas-Quilômetro (bilhões)")

    # Dados processados e unificados dos arquivos CSV enviados (TKM)
...
with st.expander("📊 Clique aqui para abrir a Tabela de Dados Absolutos"):
    ...
    # Usa a variável 'paises_para_mostrar' que já existe na lógica do seu App
    ...
    st.markdown("Valores expressos em **bilhões de toneladas-quilômetro (tkm)**.")
    st.dataframe(
        df_tkm_filtrado.set_index('Pais'), 
        use_container_width=True,
        height=(len(df_tkm_filtrado) * 35) + 38 # Altura dinâmica
    )

else:
    # Caso nenhum dado seja carregado (fallback)

...
if not df_plot.empty:
    ...
    # [LÓGICA DE TAMANHO VISUAL]: Se valor < 2%, usa 2% para a bolinha não sumir
    # O valor real continua intacto nos dados
    df_plot['Tamanho_Visual'] = df_plot['Aquaviário (%)'].apply(lambda x: 2 if x < 2 else x)
...
if not df_plot.empty:
    ...
    fig = px.scatter(
        ...
        x="Ferroviário (%)", 
        y="Rodoviário (%)", 
        size="Tamanho_Visual", # Usa tamanho mínimo visual
        color="Pais", 
        text="Pais", 
        ...
        # HOVER_DATA mostra o valor real, não o visual
        hover_data={
            "Pais": False,
    ...
    # EIXOS E LEGENDAS MAIS CLAROS COM DTICK=10
    fig.update_layout(

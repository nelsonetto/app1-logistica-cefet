...
# --- 1. DADOS (Cópia exata do arquivo) ---
...
def carregar_dados():
    ...
    # Mapeamento CORRIGIDO para garantir acentuação correta
    modal_map = {
        'Ferroviario - Freight transport': 'Ferroviário (%)',
        'Rodoviario -  Freight transport': 'Rodoviário (%)',
        'Rodoviario - Freight transport': 'Rodoviário (%)',
        'Aquaviario -  Freight transport': 'Aquaviário (%)', # Corrigido aqui
...
# Verificação extra para evitar erro de coluna
cols_necessarias = ['Ferroviário (%)', 'Rodoviário (%)', 'Aquaviário (%)']
if not df_ano.empty and all(col in df_ano.columns for col in cols_necessarias):
    ...
    st.error(f"Erro: Dados incompletos para o ano selecionado. Verifique se as colunas {cols_necessarias} existem.")
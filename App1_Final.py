...
# --- FUNÇÃO DE ORDENAÇÃO (CORRIGE POSIÇÃO DA ÁUSTRIA) ---
def normalizar_para_ordenacao(texto):
    """Remove acentos e minúsculas para ordenar corretamente (Áustria junto com A)"""
...
# Listas de Controle (PT-BR) - ORDEM ALFABÉTICA
...
# Ordenação correta da lista lateral (ignorando acentos para Áustria ficar no topo)
...
# Ordenação correta também para a lista final do gráfico
...
# FORÇA A ORDENAÇÃO DO DATAFRAME DE PLOTAGEM PARA A LEGENDA FICAR CORRETA
...
# AVISO DE INTERATIVIDADE ADICIONADO
st.info("💡 **Dica:** Clique no nome do país na legenda (à direita) para ocultá-lo ou exibi-lo no gráfico.")
...
if not df_plot.empty:
    fig = px.scatter(
        ...
        # Garante que a ordem da legenda siga a ordem do dataframe (já ordenado)

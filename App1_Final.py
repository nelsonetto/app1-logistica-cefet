...
# ==============================================================================
# 2. DEFINIÇÕES GLOBAIS (VARIÁVEIS, CORES E DADOS)
# ==============================================================================
...
# ==============================================================================
# 4. CARREGAMENTO E PROCESSAMENTO DE DADOS
# ==============================================================================
@st.cache_data
def carregar_dados_completos():
    ...
    # Função para corrigir valores pequenos
    ...
    for col in ['Ferroviário (%)', 'Rodoviário (%)', 'Aquaviário (%)']:
        if col in df_real.columns:
            # Multiplicar por 100 se estiver em decimal
    ...
    # 2. DEFINIR DADOS INICIAIS DENTRO DA FUNÇÃO PARA EVITAR ERRO DE ESCOPO
    # (Redefinido aqui para garantir que o cache não perca a referência)
    DADOS_CENARIO_INICIAL_LOCAL = [
        ('Alemanha', 17.5, 65.0, 17.5), ('Bélgica', 15.0, 75.0, 10.0), ('Brasil', 20.0, 67.5, 12.5),
        ('Canadá', 67.5, 22.5, 10.0), ('China', 40.0, 15.0, 45.0), ('Dinamarca', 12.5, 77.5, 10.0),
        ('EUA', 45.0, 35.0, 20.0), ('França', 20.0, 75.0, 5.0), ('Hungria', 30.0, 60.0, 10.0), ('Rússia', 60.0, 10.0, 30.0)
    ]
    df_ini = pd.DataFrame(DADOS_CENARIO_INICIAL_LOCAL, columns=['Pais', 'Ferroviário (%)', 'Rodoviário (%)', 'Aquaviário (%)'])
    df_ini['Ano'] = 'Inicial'
...
if paises_aviso_ativos:
    st.sidebar.warning(f⚠️ **Atenção:** Países com baixa participação aquaviária (< 1%): {', '.join(paises_aviso_ativos)}")
...
# ==============================================================================
# 6. LAYOUT PRINCIPAL (BOTÃO, SLIDER, GRÁFICO)
# ==============================================================================
st.divider()
...
if not df_plot.empty:
    ...
    with st.expander("📊 Clique aqui para abrir a Tabela de Dados Absolutos"):
        if st.session_state.slider_principal == "Inicial":
            ...
            # Garantir que as colunas de ano sejam strings
            ...
            if ano_selecionado in df_tkm.columns:
                df_final = df_tkm[['Pais', 'Modal', ano_selecionado]].copy()
